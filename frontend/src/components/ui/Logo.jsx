/**
 * Brand mark. Renders the Elevate "E" inline so callers can size it via
 * the `size` prop and so the gradient survives in browsers that strip
 * <use href> from external SVG sprites.
 *
 * The gradient `id` is suffixed with a stable instance counter so two
 * Logo elements on the same page (e.g. sidebar + topbar) never collide.
 */

let counter = 0;

export default function Logo({ size = 18, className = '', title = 'Elevate' }) {
  // Stable id per instance. Generated lazily so SSR and the first client
  // render agree.
  const gradId = `elevate-grad-${++counter}`;
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={gradId} x1="46.42" x2="103.1" y1="16.85" y2="154.6" gradientUnits="userSpaceOnUse">
